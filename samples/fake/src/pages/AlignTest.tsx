...
export default function AlignTest() {
  ...
  const justifies = [
    ...
    <div key={justify} className="mt-4">
      ...
      <div
        className={`flex flex-row ${justify} p-2 bg-gray-300`}
      >
        {boxes}
      </div>
    </div>
  ...
  const strItems = ["start", "center", "end"];

  const items = ["items-", "items-", "items-"].map((item, idx) => (
    <div key={item} className="p-2 ml-4">
      <Subtitle>flex flex-row {item + strItems[idx]}</Subtitle>
      <div
        className={`flex flex-row ${item + strItems[idx]}  h-20 bg-gray-300`}
      >
        {boxes}
      </div>
    </div>
  ...
  const flexColItems = ["items-", "items-", "items-"].map((item, idx) => (
    <div key={item} className="p-2 ml-4">
      <Subtitle>flex flex-col {item + strItems[idx]}</Subtitle>
      <div className={`flex flex-col ${item + strItems[idx]} h-40 bg-gray-300`}>
        {boxes}
      </div>
    </div>
  ));
